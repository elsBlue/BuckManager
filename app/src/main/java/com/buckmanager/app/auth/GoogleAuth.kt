package com.buckmanager.app.auth
...
object GoogleAuth {
    ...
    suspend fun signIn(activity: Activity): GoogleSignInOutcome {
        return try {
            ...
            Log.i(TAG, "credential class=${credential.javaClass.name} type=${credential.type}")
            ...
            val googleId = when {
                ...
                credential is CustomCredential -> {
                    try {
                        ...
                        Log.e(TAG, "parse failed for ${credential.type}", parse)
                        return GoogleSignInOutcome.Failed("Unexpected credential type: ${credential.type}")
                    }
                }
            ...
            Log.i(TAG, "sign-in ok email=$email")
            GoogleSignInOutcome.Success(email, googleId.profilePictureUri?.toString())
        } catch (_: GetCredentialCancellationException) {
            Log.w(TAG, "GetCredentialCancellationException after picker")
            GoogleSignInOutcome.Canceled
        } catch (e: GetCredentialException) {
            Log.e(TAG, "Google sign in failed: ${e.javaClass.simpleName}", e)
            GoogleSignInOutcome.Failed(
                listOfNotNull(e.javaClass.simpleName, e.message?.takeIf { it.isNotBlank() })
                    .joinToString(": ")
                    .ifBlank { "Google sign-in failed. Check Play services and OAuth client." }
            )
            ...
            GoogleSignInOutcome.Failed(e.message?.takeIf { it.isNotBlank() } ?: "Google sign-in failed")
        }
    }
    ...
    private fun resolveAccountEmail(credential: GoogleIdTokenCredential): String? {
        ...
        return try {
            ...
            Log.w(TAG, "Failed to parse email from idToken", e)
            fromId.ifBlank { null }
        }
    }
}
